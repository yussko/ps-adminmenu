...
RegisterNetEvent('ps-adminmenu:server:SetJob', function(data, selectedData)
    ...
    local src = source
    local playerId, Job, Grade = selectedData["Oyuncu"].value, selectedData["Meslek"].value, selectedData["Rütbe"].value
    local Player = QBCore.Functions.GetPlayer(playerId)
    local name = Player.PlayerData.charinfo.firstname .. ' ' .. Player.PlayerData.charinfo.lastname
    local jobInfo = QBCore.Shared.Jobs[Job]
    local grade = jobInfo["grades"][selectedData["Rütbe"].value]
...
RegisterNetEvent('ps-adminmenu:server:SetGang', function(data, selectedData)
    ...
    if not data or not CheckPerms(data.perms) then return end
    local src = source
    local playerId, Gang, Grade = selectedData["Oyuncu"].value, selectedData["Çete"].value, selectedData["Rütbe"].value
    local Player = QBCore.Functions.GetPlayer(playerId)
    local name = Player.PlayerData.charinfo.firstname .. ' ' .. Player.PlayerData.charinfo.lastname
    local GangInfo = QBCore.Shared.Gangs[Gang]
    local grade = GangInfo["grades"][selectedData["Rütbe"].value]
...
RegisterNetEvent("ps-adminmenu:server:SetPerms", function(data, selectedData)
    ...
    if not data or not CheckPerms(data.perms) then return end
    local src = source
    local rank = selectedData["Yetkiler"].value
...
RegisterNetEvent("ps-adminmenu:server:RemoveStress", function(data, selectedData)
    ...
    if not data or not CheckPerms(data.perms) then return end
    local src = source
    local targetId = selectedData['Oyuncu)'] and tonumber(selectedData['Oyuncu'].value) or src
    local tPlayer = QBCore.Functions.GetPlayer(tonumber(targetId))
