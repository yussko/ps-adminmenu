...
RegisterNetEvent('ps-adminmenu:server:BanPlayer', function(data, selectedData)
    ...
    local player =selectedData["Oyuncu"].value
    local reason = selectedData["Sebep"].value or ""
    local time = selectedData["Süre"].value
...
RegisterNetEvent('ps-adminmenu:server:WarnPlayer', function(data, selectedData)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(data.perms) then return end
    local targetId =selectedData["Oyuncu"].value
    local target = QBCore.Functions.GetPlayer(targetId)
    local reason = selectedData["Sebep"].value
...
RegisterNetEvent('ps-adminmenu:server:KickPlayer', function(data, selectedData)
    ...
    if not data or not CheckPerms(data.perms) then return end
    local src = source
    local target = QBCore.Functions.GetPlayer(selectedData["Oyuncu"].value)
    local reason = selectedData["Sebep"].value
...
RegisterNetEvent('ps-adminmenu:server:GiveMoney', function(data, selectedData)
    ...
    local src = source
    local target, amount, moneyType =selectedData["Oyuncu"].value, selectedData["Miktar"].value,
        selectedData["Para Tipi"].value
...
RegisterNetEvent('ps-adminmenu:server:GiveMoneyAll', function(data, selectedData)
    ...
    local src = source
    local amount, moneyType = selectedData["Miktar"].value, selectedData["Para Tipi"].value
    local players = QBCore.Functions.GetPlayers()
...
RegisterNetEvent('ps-adminmenu:server:CuffPlayer', function(data, selectedData)
    ...
    local target =selectedData["Oyuncu"].value
...
RegisterNetEvent('ps-adminmenu:server:ClothingMenu', function(data, selectedData)
    ...
    local src = source
    local target = tonumber(selectedData["Oyuncu"].value)
    ...
    TriggerClientEvent('buraya clothing eventinizi yazın', target)
end)
...
RegisterNetEvent("ps-adminmenu:server:setPed", function(data, selectedData)
    ...
    local ped = selectedData["Ped Modeli"].label
    local tsrc =selectedData["Oyuncu"].value
